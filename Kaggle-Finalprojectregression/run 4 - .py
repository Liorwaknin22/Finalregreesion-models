...
def preprocess_data(data):
    ...
    data.drop(
        ['host_since', 'id', 'host_id', 'property_type', 'has_availability', 'first_review', 'last_review', 'license',
         'host_is_superhost', 'host_verifications', 'host_has_profile_pic', 'host_identity_verified', 'longitude',
         'latitude', 'minimum_minimum_nights', 'minimum_maximum_nights', 'minimum_nights_avg_ntm', 'maximum_nights',
         'maximum_minimum_nights', 'maximum_maximum_nights', 'maximum_nights_avg_ntm','availability_60','availability_90'], axis=1, inplace=True)
    # Fill missing values:
    ...
    print(f'There are {len(data_scaled.columns)} columns.')
    return data_scaled


def testLogisticRegression4(train_data):
    # Load test dataset
...
id, prob = testLogisticRegression4(train_data)
